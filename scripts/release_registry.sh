...
REGISTRY_VERSION="${REGISTRY_VERSION:-1.2.1}"
EXPORT_SCHEMA_VERSION="${EXPORT_SCHEMA_VERSION:-1.1.0}"
...
python3 "$ROOT/scripts/build_registry_exports.py" \
  --source "$SOURCE" \
  --concrete "$CONCRETE" \
...
python3 "$ROOT/scripts/validate_registry.py" \
  --source "$SOURCE" \
  --concrete "$CONCRETE" \
  --public-concrete "$ROOT/variable_registry.public.concrete.csv" \
  --public-json "$EXPORTS_DIR/public_registry.json" \
  --manifest "$MANIFEST" \
  --report "$REPORT" \
  --release-status "$RELEASE_STATUS"
