...
This is the first public, release-ready HADES variable-registry version and the start of the binding public identifier-stability promise.

### Public release state

- registry and export schema version set to `1.0.0`
- `release_status` set to `public`
- `public_release_ready` set to `true`
- public concrete CSV and public JSON registry are generated by the default build
- currently active variables are marked as introduced in the 1.0.0 public lifecycle baseline

### Storage update

- large per-pixel VNIR matrix exports now use Parquet by default
- CSV remains available as an optional interchange format
- NPZ is removed from the registry storage contract

### Stable implementation references

- fluorescence/RootCam analysis: https://github.com/valerian-meline/HADES_FC
- hyperspectral analysis: https://github.com/valerian-meline/HADES_HSI
- stable HADES research-resources landing page: https://npec-nl.github.io/hades-research-resources

The resource landing page is used as the stable technical-documentation reference while the method manuscript remains under review and can later host the camera specification and light-calibration documents without changing registry references.

### Validation and release automation

- validation rejects obsolete NPZ storage references
- validation checks that HADES-native FC/VNIR method references use the public code repositories and research-resources page
- CI and release helper run the isolated strict-public validation gate
